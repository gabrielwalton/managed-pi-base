...
apt-get update
DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends git python3 python3-venv ca-certificates sudo
...
install -d -m 0755 /opt/managed-pi
...
install -d -m 0755 /opt/managed-pi/agent-source
...
systemctl enable managed-pi-firstboot.service managed-pi-app.service
